    ...
    if overlay_gt_pred:
        ...
        if show:
            if wait_time == 0:
                ...
                plt.pause(wait_time)
        if out_file is not None:
            # viz_dataset_v6_0.1/good/Australia-ASSA_20220613_031500_62_0.9.jpg'
            mmcv.imwrite(img, out_file)
            # 这里我分开保存一下gt框和检测结果框
            out_file_root, img_name = os.path.split(out_file)
            gt_out_file = os.path.join(out_file_root,'gt')
            if not os.path.exists(gt_out_file):
                os.mkdir(gt_out_file)
            mmcv.imwrite(img_with_gt, os.path.join(gt_out_file, img_name))

            det_out_file = os.path.join(out_file_root,'det')
            if not os.path.exists(det_out_file):
                os.mkdir(det_out_file)
            mmcv.imwrite(img_with_det, os.path.join(det_out_file, img_name))
