...
class MaxIoUAssigner(BaseAssigner):
    ...
    def assign_wrt_overlaps(self, overlaps, gt_labels=None):
        ...
        max_overlaps, argmax_overlaps = overlaps.max(dim=0) #dim=0 表示沿着第一个维度（行）进行操作，即在每列中找到最大值及其对应的索引。
        # for each gt, which anchor best overlaps with it
        # for each gt, the max iou of all proposals
        gt_max_overlaps, gt_argmax_overlaps = overlaps.max(dim=1) #dim=1 表示沿着第二个维度（列）进行操作，即在每行中找到最大值及其对应的索引。
