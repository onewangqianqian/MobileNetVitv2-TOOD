...
from mmdet.core import bbox_overlaps, alpha_bbox_overlaps
...
def siou_loss(pred, target, eps=1e-7):
    r""" SIoU Loss https://arxiv.org/pdf/2205.12740.pdf
    ...
    sious = ious - (0.5 * distance_cost + shape_cost) + eps  # SIou
...
def eiou_loss(pred, target, eps=1e-7):
    ...
    # DIoU
    eious = ious - rho2 / c2 - rho_w2 / cw2 - rho_h2 / ch2
    loss = 1 - eious
    return loss
...
@mmcv.jit(derivate=True, coderize=True)
@weighted_loss
def alphaiou_loss(pred, target, eps=1e-7, alphafactor=3,
    ...
    # iou = inter / union
    ious = torch.pow(overlap / union + eps, alphafactor)
    beta = 2 * alphafactor
    ...
    if GIoU or DIoU or CIoU or EIoU:
        if CIoU or DIoU or EIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            if DIoU:
                alpha_ious = ious - rho2 / c2  # DIoU
            if EIoU:
                alpha_ious = ious - rho2 / c2 - rho_w2 / cw2 - rho_h2 / ch2
            elif CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi ** 2) * torch.pow(torch.atan(w2 / h2) - torch.atan(w1 / h1), 2)
                with torch.no_grad():
                    alpha_ciou = v / ((1 + eps) - overlap / union + v)
                # return iou - (rho2 / c2 + v * alpha_ciou)  # CIoU
                alpha_ious = ious - (rho2 / c2 + torch.pow(v * alpha_ciou + eps, alphafactor))  # CIoU
        else:  # GIoU https://arxiv.org/pdf/1902.09630.pdf
            c_area = cw * ch + eps  # convex area
            # return iou - (c_area - union) / c_area  # GIoU

            alpha_ious = ious - torch.pow((c_area - union) / c_area + eps, alphafactor)  # GIoU
    else:
        alpha_ious = ious  # torch.log(iou+eps) or iou

    loss = 1 - alpha_ious.clamp(min=-1.0, max=1.0)
