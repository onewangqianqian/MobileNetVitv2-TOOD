...
class TaskDecomposition(nn.Module):
    ...
    def __init__(self,
        ...
        # self.ppyoloe_selayer = PPYOLOESELayer(self.feat_channels*self.stacked_convs)
    ...
    def forward(self, feat, avg_feat=None):
        ...
        # assert 1==2,'{}'.format(weight.shape) #torch.Size([4, 6, 1, 1])
...
class TOODHead(ATSSHead):
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            # extract task interactive features 任务交互特征
            inter_feats = []
            ...
            # 通过特征提取器得到丰富的多尺度特征
            # 并用于送入到后续两个TAP模块中进行分类与定位对齐

            # task decomposition
            # 采用注意力机制进行任务分解，提取任务相关特征
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
            cls_feat = self.cls_decomp(feat, avg_feat)
            reg_feat = self.reg_decomp(feat, avg_feat)

            # 预测阶段，进一步对两个任务通过调整空间分布进行自适应对齐
            # cls prediction and alignment
            # 空域概率图M(R_H*W*1)调整分类预测
            ...
            # 空域移位图O（R_H*W*8）调整每个位置的预测目标位置信息
            ...
            # print('reg_bbox:',reg_bbox.shape)
            # print('reg_offset:',reg_offset.shape)
            # shape:
            # feat: torch.Size([4, 1536, 40, 40])
            # avg_feat: torch.Size([4, 1536, 1, 1])
            # cls_feat: torch.Size([4, 256, 40, 40])
            # cls_logits: torch.Size([4, 80, 40, 40])
            # cls_prob: torch.Size([4, 1, 40, 40])
            # reg_feat: torch.Size([4, 256, 40, 40])
            # reg_bbox: torch.Size([4, 4, 40, 40])
            # reg_offset: torch.Size([4, 8, 40, 40])
