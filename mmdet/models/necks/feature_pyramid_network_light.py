...
class FeaturePyramidNetwork(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.extra_blocks = ConvModule(
            in_channels=512,
            out_channels=1024,
            kernel_size=3,
            stride=2,
            padding=1)
        self.extra_blocks2 = ConvModule(
            in_channels=128,
            out_channels=128,
            kernel_size=3,
            stride=2,
        ...
        # 经过SSF后的1x1卷积
        self.SSF_C5 = nn.Conv2d(256, 128, 1)
        self.SSF_C4 = nn.Conv2d(128, 128, 1)
        ...
        # [256, 384, 512, 1024],
        # [256,512,1024]
        self.conv_1x1_4 = nn.Conv2d(512, 128, 1)
        self.conv_1x1_3 = nn.Conv2d(384, 128, 1)
        self.conv_1x1_2 = nn.Conv2d(256, 128, 1)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                xavier_init(m, distribution='uniform')
    ...
    def forward(self, x):
        ...
        # ------------------------------- #
        C2, C3, C4 = x
        C5 = self.extra_blocks(C4)

        # ------------------------------- #
        # 得到SCE模块的输出
        # ------------------------------- #
        SCE_out = self.SCE(C5)
        # print(C2.shape) torch.Size([8, 256, 40, 40])
        # print(C3.shape) torch.Size([8, 384, 20, 20])
        # print(C4.shape) torch.Size([8, 512, 10, 10])
        # print(C5.shape) torch.Size([8, 1024, 5, 5])
        # print(SCE_out.shape) torch.Size([8, 128, 10, 10])
        ...
        for i in [R2, R3, R4, R5]:
            outs.append(i)
        outs.append(self.extra_blocks2(outs[-1]))
        return outs
